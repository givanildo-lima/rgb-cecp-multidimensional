"""
==============================================================================
Análise Comparativa: Abordagem Multicanal vs Canais R, G e B Individuais
...
ARQUIVOS ESPERADOS NA PASTA:
    mv_*.xlsx   r_*.xlsx   g_*.xlsx   b_*.xlsx   unv_*.xlsx
    (4 arquivos por prefixo × 4 tipos de ruído = 20 arquivos no total)

NOTA SOBRE PARÂMETROS:
    Todos os canais individuais (R, G, B) e o Monocromático são calculados
    com ordpy.complexity_entropy(dx=2, dy=2), garantindo paridade com o
...
ABORDAGEM_LABELS = {
    ...
    "Canal G":       "Canal G",
    "Canal B":       "Canal B",
    "Monocromatico": "Monocromático (dx=dy=2)",
}
...
ABORDAGEM_ORDER = ["Multicanal", "Canal R", "Canal G", "Canal B", "Monocromatico"]
...
def plot_scatter_comparativo(dfs_full, dfs_filt, noises, outfile):
    ...
    fig.suptitle(
        "Espaço de Características — Multicanal, Canais R, G, B e Monocromático\n"
...
def plot_scatter_por_imagem(dfs_full, dfs_filt, noises, outfile):
    ...
    fig.suptitle(
        "Espaço de Características por Imagem — Multicanal, Canais R, G, B e Monocromático\n"
        "(centróides e elipses calculados sem I=0)",
...
def plot_barras_distancias(dfs_filt, noises, outfile):
    ...
    for abord in ["Canal R", "Canal G", "Canal B", "Monocromatico"]:
...
def plot_trajetoria_intensidades(dfs_full, dfs_filt, noises, outfile):
    ...
    fig.patch.set_facecolor(BG_DARK)
    gs = GridSpec(5, 4, figure=fig, hspace=0.38, wspace=0.28,
    ...
    fig.suptitle(
        ...
        "Multicanal, Canais R, G, B e Monocromático  ·  (◇ = referência I=0 · ★ = centróide sem I=0)",
        color=TEXT_MAIN, fontsize=13, fontweight="bold", y=0.975)
...
def main():
    ...
    print("=" * 62)
    print("  Análise: Multicanal vs Canais R, G, B e Monocromático")
    ...
    plot_scatter_comparativo(
        ...
        outfile=os.path.join(out, "fig1_scatter_comparativo_canais.png"))

    plot_scatter_por_imagem(
        dfs_full, dfs_filt, noises,
        outfile=os.path.join(out, "fig2_scatter_por_imagem_canais.png"))

    plot_barras_distancias(
        dfs_filt, noises,
        outfile=os.path.join(out, "fig3_barras_distancias_canais.png"))

    plot_trajetoria_intensidades(
        dfs_full, dfs_filt, noises,
        outfile=os.path.join(out, "fig4_trajetoria_canais.png"))
